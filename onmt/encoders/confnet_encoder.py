...
class ConfnetEncoder(EncoderBase):
    ...
    def forward(self, confnets, scores, src_lengths=None, par_arc_lengths=None):
        ...
        output_list = torch.tensor([]).to(self.device)#cuda()
        #### FEATS NOT SUPPORTED ###
        ...
        self_attentions = torch.tensor([]).to(self.device)#cuda()
        for em, score, lengths in zip(emb_trans, scores_, par_arc_lengths_):
            ...
            mask = torch.arange(max_par_arcs)[None, :].to(self.device) < lengths[:, None].to(self.device).type(torch.float)
            ...
            final_attention = attention.masked_fill(torch.isnan(attention), 0) #(batch, max_par_arc)
            ...
            self_attentions = torch.cat((self_attentions, final_attention.unsqueeze(0)), dim=0)
        # a = self.dropout(a)

        #self_attentions = self_attentions.permute(1, 0, 2) #(batch, max_sent_len, max_par_arc)
        ...
        return encoder_final, memory_bank, src_lengths, self_attentions
