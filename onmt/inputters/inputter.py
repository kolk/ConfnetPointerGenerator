...
def make_src_confnet(data, vocab):
    ...
    max_par_arc_size = max([t.size(1) for t in data_])
    src_vocab_size = max([p.max() for t in data_ for p in t]) + 1
    alignment = torch.zeros(src_size, len(data_), max_par_arc_size, src_vocab_size) #slen, batch, vocab
    for i, sent in enumerate(data_):
        for j, par_arcs in enumerate(sent):
            for k, t in enumerate(par_arcs):
                alignment[j, i, k, t] = 1
    return alignment
